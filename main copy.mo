...
shared ({ caller = initializer }) actor class RWA() = this {
    ...
    private stable var assets = HashMap.HashMap<AssetId, Asset>(0, Nat.equal, natHash);
private stable var shares = HashMap.HashMap<ShareId, Share>(0, Nat.equal, natHash);
private stable var transactions = HashMap.HashMap<Nat, Transaction>(0, Nat.equal, natHash);
